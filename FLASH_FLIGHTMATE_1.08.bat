...
echo FlightMate 1.08 默认升级：保留 NVS 设置并写入程序。
"%TOOL%" --chip esp32s3 --port "%PORT%" --baud 921600 --before default-reset --after hard-reset write-flash --compress ^
    0x0 "FlightMate-1.08-bootloader.bin" ^
    0x8000 "FlightMate-1.08-partitions.bin" ^
    0xe000 "FlightMate-1.08-boot_app0.bin" ^
    0x10000 "FlightMate-1.08-app.bin"
if errorlevel 1 goto fail
goto success

:fresh
echo FlightMate 1.08 全新烧录：将擦除设备全部 Flash 数据。
echo 按 Ctrl+C 可取消，按任意键继续。
pause >nul
"%TOOL%" --chip esp32s3 --port "%PORT%" --baud 921600 --before default-reset erase-flash
if errorlevel 1 goto fail
"%TOOL%" --chip esp32s3 --port "%PORT%" --baud 921600 --before default-reset --after hard-reset write-flash --compress ^
    0x0 "FlightMate-1.08-merged-16MB.bin"
if errorlevel 1 goto fail
goto success

:success
echo [完成] FlightMate 1.08 已烧录，设备将自动重启。
pause
exit /b 0
